...
namespace CompanioNationAPI
{
    public class Database
    {
        ...
        /// <summary>
        /// Add or subtract subscription days by email.
        /// </summary>
        public async Task<ResponseWrapper<bool>> AddSubscriptionDaysByEmailAsync(string email, int daysToAdd)
        {
            if (string.IsNullOrWhiteSpace(email))
            {
            ...
            try
            {
                using (var conn = new SqlConnection(_connectionString))
                {
                    ...
                    using (var cmd = new SqlCommand("cn_add_subscription_days", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@email", email);
                        cmd.Parameters.AddWithValue("@days_to_add", daysToAdd);
            ...
            {
                ErrorLog.LogErrorException(ex, $"Error updating subscription days for email {email}");
